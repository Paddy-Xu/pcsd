...
&&&&\\ 
\texttt{TRAN\_ID} & \texttt{Status} & \texttt{lastLSN} \\
\texttt{T1} & \texttt{progress} & \texttt{4} \\ 
\texttt{T2} & \texttt{progress} & \texttt{9} \\ 
\texttt{T3} & \texttt{committed} & \texttt{10} \\ 
\end{tabular*}
...
\label{dirty}
\end{table}
The transaction table, see table~\ref{trans}, is based on the log, see table~\ref{log}. Transaction three has committed, whilst transaction one and two are still in progress, when the crash occurs.\\
